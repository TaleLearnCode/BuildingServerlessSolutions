...
resource "azurerm_servicebus_subscription" "nextcoreintransit_notificationmanager" {
  name                                      = "${module.service_bus_topic_subscription.name.abbreviation}-CoolRevive-NCIT-NotificationManager${var.resource_name_suffix}-${var.azure_environment}-${module.azure_regions.region.region_short}"
